...
if "estado_cargado" not in st.session_state:
    ...
    if estado:
        st.session_state["productos_cacheados"] = estado.get("productos_cacheados", [])
        st.session_state["precios_anteriores"] = estado.get("precios_anteriores", {})
        st.session_state["ultima_actualizacion"] = estado.get("ultima_actualizacion")
        st.session_state["productos_aumentados"] = estado.get("productos_aumentados", [])
        st.session_state["hubo_aumento"] = estado.get("hubo_aumento", False)
        st.session_state["productos_mostrados"] = estado.get("productos_mostrados", [])
        st.session_state["historial_aumentos"] = estado.get("historial_aumentos", [])

        reglas = estado.get("reglas", [])
        st.session_state["reglas"] = pd.DataFrame(reglas) if reglas else obtener_reglas_iniciales()
